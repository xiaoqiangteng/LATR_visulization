...
class MultiHeadQueryEmbedding(nn.Module):
    def __init__(self, num_query=800, embed_dim=256):
        super().__init__()
        self.struct_query = nn.Embedding(num_query, embed_dim)
        self.loc_query = nn.Embedding(num_query, embed_dim)
        self.cls_query = nn.Embedding(num_query, embed_dim)

    def forward(self, B):
        q_struct = self.struct_query.weight.unsqueeze(0).repeat(B, 1, 1)  # [B, N, C]
        q_loc = self.loc_query.weight.unsqueeze(0).repeat(B, 1, 1)
        q_cls = self.cls_query.weight.unsqueeze(0).repeat(B, 1, 1)
        return q_struct, q_loc, q_cls
...
class LATRHead(nn.Module):
    def __init__(self, args,
        ...
        self.struct_token_extractor = StructuredLaneEmbedding(embed_dims, embed_dims)
        # self.multihead_query_embed = MultiHeadQueryEmbedding(num_query=self.num_query, embed_dim=self.embed_dims)
        self.multihead_query_embed = MultiHeadQueryEmbedding(num_query=800, embed_dim=self.embed_dims)
    ...
    def forward(self, input_dict, is_training=True):
        ...
        struct_token = self.struct_token_extractor(img_feats[0])  # [B, 1, C]
        query_embeds = query_embeds + struct_token
        ...
        # refined_queries = self.query_refinement(filtered_queries, additional_features=fused_features)
