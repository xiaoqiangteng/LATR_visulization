...
def plot_3D(lane_prediction, lane_visibility_prediction, lane_ground_truth, lane_visibility_ground_truth,
    ...
    for i in range(len(lane_prediction)):
        ...
        if data_list_P_g2im.shape[1] == 3:
            ...
            x_2d, y_2d = projective_transformation(data_list_P_g2im, data_list_lane_prediction[:, 0], data_list_lane_prediction[:, 1], data_list_lane_prediction[:, 2])
            x_2d = x_2d.astype(np.int32)
            y_2d = y_2d.astype(np.int32)
...
def plot_IPM(lane_prediction, lane_visibility_prediction, lane_ground_truth, lane_visibility_ground_truth,
            data_list_P_g2im, H_im2ipm, output_file_name=None, file_name_image=None, data_list_error=None):
    if len(lane_ground_truth) == 0 or output_file_name is None:
        return
...
def get_data_visulization(file_path, file_path_image):
    ...
    for i, data in enumerate(tqdm(data_list_file, desc="Processing data")):
        ...
        cam_extrinsics = np.array(data_dict_lane['cam_extrinsics'])
        cam_intrinsics = np.array(data_dict_lane['cam_intrinsics'])
        ...
        transform = np.matmul(H_crop, P_g2im)
